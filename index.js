...
var gutil = require('gulp-util');  // added by theConscience

var PLUGIN_NAME = 'gulp-sourcemap';
var urlRegex = /^(https?|webpack(-[^:]+)?):\/\//;

var origFilePath = '';  // added by theConscience
...
module.exports.init = function init(options) {
  function sourceMapInit(file, encoding, callback) {
    ...
    origFilePath = file.path;  // added by theConscience
    gutil.log('sourceMapInit: origFilePath', gutil.colors.bold.gray(origFilePath));  // added by theConscience
...
module.exports.write = function write(destPath, options) {
  ...
  function sourceMapWrite(file, encoding, callback) {
    ...
    gutil.log('sourceMapWrite: origFilePath', gutil.colors.bold.gray(origFilePath));  // added by theConscience
    file.newPath = file.path;  // added by theConscience
    gutil.log('sourceMapWrite: file.newPath', gutil.colors.bold.gray(file.newPath));  // added by theConscience
    ...
    if (!destPath) {
      ...
      sourceMapFile.oldPath = origFilePath;  // added by theConscience
      sourceMapFile.newPath = file.newPath;  // added by theConscience
      gutil.log('sourceMapWrite: sourceMapFile.oldPath', gutil.colors.bold.gray(sourceMapFile.oldPath));  // added by theConscience
      gutil.log('sourceMapWrite: sourceMapFile.newPath', gutil.colors.bold.gray(sourceMapFile.newPath));  // added by theConscience
